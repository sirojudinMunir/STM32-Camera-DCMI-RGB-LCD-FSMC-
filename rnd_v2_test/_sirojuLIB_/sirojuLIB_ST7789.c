...
{
	uint8_t ram, temp;
	temp = *(__IO uint8_t*)(((uint32_t)0x60040000));
	ram = (temp & 0xfc) | ((temp << 1) & 2) | ((temp >> 1) & 1);
	return ram;
}

void ST7789_SendCmd(uint8_t x) {
    ST7789_CMD = ((x & 0xFC) | ((x & 0x01) << 1) | ((x & 0x02) >> 1));
}

void ST7789_SendData(uint8_t x) {
    ST7789_DATA = ((x & 0xFC) | ((x & 0x01) << 1) | ((x & 0x02) >> 1));
}
...
void ST7789_drawChar(int16_t x, int16_t y, unsigned char c, uint16_t color, uint16_t bg, uint8_t size)
{
	...
	for (uint8_t i = 0; i < 6; i++)
	{
		...
		{
		  if (line & 0x1) ST7789_DrawPixel((x+i)*size, (y+j)*size, color);
		  else ST7789_DrawPixel((x+i)*size, (y+j)*size, bg);
		  line >>= 1;
		}
	}
}
...
void ST7789_printImage(uint16_t x, uint16_t y, uint16_t w, uint16_t h, const uint8_t *data, RGB565_mode_t mode)
{
	...
	{
		//ST7789_SetCursorPosition(x, i+y, w+x-1, i+y);
		for (uint16_t n  = 0; n < w*2; n+=2)
		{
			if (mode == _MSB_FIRST) {
				ST7789_SendData(data[n + w*2*i]);
				ST7789_SendData(data[n+1 + w*2*i]);
			}
			else if (mode == _LSB_FIRST) {
				ST7789_SendData(data[n+1 + w*2*i]);
				ST7789_SendData(data[n + w*2*i]);
			}
		}
